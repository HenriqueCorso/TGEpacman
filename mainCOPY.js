...
const movePlayer = () => {
  ...
  // Move up
  if (keys.up && lastKey === 'w' && row > 0) {
    const targetRow = row - 1;
    const targetCol = col;
    movePlayerToTarget(player, targetRow, targetCol, tileSize);
  }
  // Move left
  else if (keys.left && lastKey === 'a' && col > 0) {
    const targetRow = row;
    const targetCol = col - 1;
    movePlayerToTarget(player, targetRow, targetCol, tileSize);
  }
  // Move down
  else if (keys.down && lastKey === 's' && row < map.length - 1) {
    const targetRow = row + 1;
    const targetCol = col;
    movePlayerToTarget(player, targetRow, targetCol, tileSize);
  }
  // Move right
  else if (keys.right && lastKey === 'd' && col < map[row].length - 1) {
    const targetRow = row;
    const targetCol = col + 1;
    movePlayerToTarget(player, targetRow, targetCol, tileSize);
  }
...
const movePlayerToTarget = (player, targetRow, targetCol, tileSize) => {
